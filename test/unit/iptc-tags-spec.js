...
// The private getResourceBlock, getNaaResourceBlock, and readTag functions
// are exercised through the public IptcTags.read with tag data wrapped in
...
import IptcTagNames from '../../src/iptc-tag-names.js';
...
describe('iptc-tags', function () {
    ...
    it('should read an IPTC resource block', () => {
        // The block size covers only the first tag so finding exactly one tag
        // proves that the size field of the resource block was parsed.
        const dataView = getDataView('8BIM\x04\x04\x00\x00\x00\x00\x00\x07' + '\x1c\x47\x11\x00\x02BC' + '\x1c\x47\x12\x00\x02DE');
    ...
    it('should find a single IPTC NAA resource block', () => {
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
    });

    it('should find the IPTC NAA resource block without previous padding', () => {
        const dataView = getDataView('8BIM\x04\x05\x00\x00\x00\x00\x00\x02\x00\x00' + '8BIM\x04\x04\x00\x00\x00\x00\x00\x07' + '\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
    });

    it('should find the IPTC NAA resource block with previous padding', () => {
        const dataView = getDataView('8BIM\x04\x05\x00\x00\x00\x00\x00\x01\x00\x00' + '8BIM\x04\x04\x00\x00\x00\x00\x00\x07' + '\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
    });

    it('should find the IPTC NAA resource block with non-empty resource name (without padding)', () => {
        const dataView = getDataView('8BIM\x04\x04\x05aname\x00\x00\x00\x07' + '\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
    });

    it('should find the IPTC NAA resource block with non-empty resource name (with padding)', () => {
        const dataView = getDataView('8BIM\x04\x04\x04name\x00\x00\x00\x00\x07' + '\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
    });
    ...
    it('should read IPTC NAA resource tag', () => {
        restores.push(swapProperties(IptcTagNames.iptc, {0x4711: 'MyIptcTag'}));
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0);
        expect(tags['MyIptcTag'].id).to.equal(0x4711);
        expect(tags['MyIptcTag'].value).to.deep.equal([0x42, 0x43]);
        expect(tags['MyIptcTag'].description).to.equal('BC');
    });

    it('should read encoded ASCII IPTC NAA resource tag', () => {
        restores.push(swapProperties(IptcTagNames.iptc, {0x4711: 'MyIptcTag'}));
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x04\x41\xc3\xba\x43');
        const tags = IptcTags.read(dataView, 0);
        expect(tags['MyIptcTag'].description).to.equal('AúC');
    });
    ...
    it('should read IPTC NAA resource tag with dynamic name', () => {
        ...
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x01\x42');

        const tags = IptcTags.read(dataView, 0);

        expect(tags['MyIptcTag'].id).to.equal(0x4711);
        expect(tags['MyIptcTag'].description).to.deep.equal([0x42]);
    });
    ...
    it('should read IPTC NAA resource tag with dynamic description', () => {
        ...
        const tags = IptcTags.read(dataView, 0);
        expect(tags['MyIptcTag'].id).to.equal(0x4711);
        expect(tags['MyIptcTag'].value).to.deep.equal([0x42]);
        expect(tags['MyIptcTag'].description).to.equal(1);
    });
    ...
    it('should handle IPTC NAA resource tag with dynamic description that throws', () => {
        ...
        expect(tags['MyIptcTag'].id).to.equal(0x4711);
        expect(tags['MyIptcTag'].value).to.deep.equal([0x42]);
        expect(tags['MyIptcTag'].description).to.equal('B');
    });

    it('should read included undefined IPTC NAA resource tag', () => {
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x02\x42\x43');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags['undefined-18193'].id).to.equal(0x4711);
        expect(tags['undefined-18193'].value).to.deep.equal([0x42, 0x43]);
        expect(tags['undefined-18193'].description).to.deep.equal([0x42, 0x43]);
    });
    ...
    it('should read multiple IPTC NAA resource tags', () => {
        ...
        const dataView = getNaaBlockDataView('\x1c\x47\x11\x00\x02BC' + '\x1c\x47\x12\x00\x02DE');
        const tags = IptcTags.read(dataView, 0);
        expect(tags['MyIptcTag1'].id).to.equal(0x4711);
        expect(tags['MyIptcTag1'].value).to.deep.equal([0x42, 0x43]);
        expect(tags['MyIptcTag1'].description).to.equal('BC');
        expect(tags['MyIptcTag2'].id).to.equal(0x4712);
        expect(tags['MyIptcTag2'].value).to.deep.equal([0x44, 0x45]);
        expect(tags['MyIptcTag2'].description).to.equal('DE');
    });

    it('should abort reading a tag when the tag is faulty', () => {
        const dataView = getNaaBlockDataView('\x00\x47\x11\x00\x02BC');
        const tags = IptcTags.read(dataView, 0, true);
        expect(tags).to.deep.equal({});
    });
    ...
    it('should read and decode IPTC tag with encoding', () => {
        ...
        restores.push(swapProperties(TagDecoder, {
